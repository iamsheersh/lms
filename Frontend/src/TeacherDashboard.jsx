...
import { Moon, Sun, CheckCircle2 } from 'lucide-react'; // Added CheckCircle2
...
const TeacherDashboard = () => {
  ...
  const [questions, setQuestions] = useState([
    { id: 1, type: 'mcq', question: '', options: ['', '', '', ''], correctOption: 0 } // Changed 'correct' to 'correctOption'
  ]);
  ...
  // Helper to update specific question fields
  ...
  const handlePublishTest = async () => {
    ...
    try {
      ...
      const testData = {
        creator_id: currentUser.uid, // ER Diagram: creator_id FK
        test_name: testTitle,        // ER Diagram: test_name
        topic: "General",            // ER Diagram: topic
        max_score: 100,              // ER Diagram: max_score
  ...
  const handlePublishMaterial = async () => {
    ...
    setPublishing(true);
    try {
      // Get current user for uploader_id
      const currentUser = auth.currentUser;
      if (!currentUser) {
      ...
      const contentData = {
        uploader_id: currentUser.uid,  // ER Diagram: uploader_id FK
        title: contentTitle,           // ER Diagram: title
        topic: "General",              // ER Diagram: topic
        type: contentCategory,         // ER Diagram: type (video/pdf/link)
        url: selectedFile ? URL.createObjectURL(selectedFile) : '#', // ER Diagram: url
        ...
        {activeTab === 'upload' && (
          <div className="max-w-4xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className={`p-8 rounded-[2rem] shadow-xl border space-y-8 transition-colors ${isDark ? 'bg-slate-900 border-slate-800 shadow-none' : 'bg-white border-slate-100 shadow-slate-200/50'}`}>
              ...
              >
                {publishing ? 'Publishing...' : 'Publish Material'}
              </button>
            </div>
          </div>
        )}
          ...
          <div className="max-w-4xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className={`p-8 md:p-10 rounded-[2.5rem] shadow-xl border space-y-10 transition-colors ${isDark ? 'bg-slate-900 border-slate-800 shadow-none' : 'bg-white border-slate-100 shadow-slate-200/50'}`}>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {q.options.map((opt, optIdx) => (
                          <div key={optIdx} className="relative group flex items-center gap-2">
                            {/* NEW: Mark Correct Answer Button */}
                            <button
                              type="button"
              ...
              <div className="flex flex-col md:flex-row gap-4 pt-4">
                ...
                >
                  {publishing ? 'Publishing...' : 'Publish Test Now'}
                </button>
              </div>
            </div>
          </div>
